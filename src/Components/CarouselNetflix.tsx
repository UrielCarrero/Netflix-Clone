import React, {useState, useRef, ReactNode, useEffect, useLayoutEffect} from 'react';
import Slider from 'react-slick';
import {CarouselModal} from '../Components/CarouselModal';
import '../Styles/CarouselNetflix.css'
...
const imgNumbers=[
  <svg id="rank-1" viewBox="-20 0 70 154" className='svgcarousel__number'>
  <path stroke="#595959" stroke-linejoin="square" stroke-width="4" d="M35.377 152H72V2.538L2 19.362v30.341l33.377-8.459V152z"></path>
  </svg>, 
  <svg id="rank-2" viewBox="0 0 80 154" className='svgcarousel__number'>
  ...
  <svg id="rank-3" viewBox="0 0 80 154" className='svgcarousel__number'>
  ...
  <svg id="rank-4" viewBox="0 0 81 154" className='svgcarousel__number' >
  ...
  <svg id="rank-5" viewBox="0 0 81 154" className='svgcarousel__number'>
  ...
  <svg id="rank-6" viewBox="0 0 81 154" className='svgcarousel__number'>
  ...
  <svg id="rank-7" viewBox="0 0 78 154" className='svgcarousel__number'>
  ...
  <svg id="rank-8" viewBox="0 0 77 154" className='svgcarousel__number'>
  ...
  <svg id="rank-9" viewBox="0 0 71 154" className='svgcarousel__number' >
  ...
  <svg id="rank-10" viewBox="0 0 140 154" className='svgcarousel__number' >
...
export const CarouselNetflix = ({isTop ,movies, title, changeModalState , 
  gatherDescription, page, addToFavorites, user_favorites}:ICarrouselNetflix):JSX.Element => {
    ...
    useEffect(()=>{
      ...
      if(windowWidth !== undefined){
        ...
        if(windowWidth >= 992){
          ...
          if(isTop)
          {
            setSlideDimensions({width: 100/5 , height: slideWidth*0.7119/5})
          }
          else{
            setSlideDimensions({width: 100/5 , height: slideWidth*0.5655/5})
          }
        }
          ...
          if(isTop)
          {
            setSlideDimensions({width: 100/4 , height: slideWidth*0.7119/4})
          }
          else{
            setSlideDimensions({width: 100/4 , height: slideWidth*0.5655/4})
          }
        }
          ...
          if(isTop)
          {
            setSlideDimensions({width: 100/3 , height: slideWidth*0.7119/3})
          }
          else{
            setSlideDimensions({width: 100/3 , height: slideWidth*0.5655/3})
          }
        }
        else if(windowWidth < 576){
          ...
          if(isTop)
          {
            setSlideDimensions({width: 100/2 , height: slideWidth*0.7119/2})
          }
          else{
            setSlideDimensions({width: 100/2 , height: slideWidth*0.5655/2})
          }
        }
        else{
      ...
      const SlickArrowLeft = ({ currentSlide, slideCount, ...props }:any) => (
        isScrolled?
        <span className='prev__button' style={windowWidth!==undefined?{width:`${0.044477*windowWidth}px`}:{width:"60px"}}>
          <button
            {...props}
            className={
      ...
      const SlickArrowRight = ({ currentSlide, slideCount, ...props }:any) => (
          isScrolled?
          <span className='next__button' style={windowWidth!==undefined?{width:`${0.044477*windowWidth}px`}:{width:"60px"}}>
            <button
              {...props}
              className={
              ...
              style={{height:"100%"}}
            >
              <i style={windowWidth!==undefined?{width:`${0.044477*windowWidth}px`}:{}} className="ri-arrow-right-s-line"></i>
            </button>
          </span>:
          <span className='next__button' style={windowWidth!==undefined?{width:`${0.044477*windowWidth}px`}:{width:"60px"}}>
            <button style={{height:"100%"}} onClick={()=>{
                                ...
                                setIsScrolled(true)}}>
              <i style={windowWidth!==undefined?{width:`${0.044477*windowWidth}px`}:{width:"60px"}} className="ri-arrow-right-s-line"></i>
            </button>
          </span>
      ...
      const settings = {
          ...
          centerMode: true,
          infinite: isScrolled,
          centerPadding: windowWidth!==undefined?`${0.044477*windowWidth}px`:"60px",
          slidesToShow: 1,
          draggable: false,
          speed: 500, 
          ...
          dots: true, 
          appendDots: (dots:ReactNode):JSX.Element =>{return(<div className='carousel__dots'>{dots}</div>)}
        };

      const renderIndividualSlides = ():Array<JSX.Element> => {
        let slidesArray = [];
        let slideWidth = windowWidth!==undefined?windowWidth - (0.1131*windowWidth):1222
        let slidesInCarrousel = windowWidth!==undefined?Math.floor(100/slideDimensions.width):5

        for( let i = 0; i < (movies.length/slidesInCarrousel); i++){
          slidesArray.push(
            <span className='carousel__span'>
              {
              movies.slice(i*slidesInCarrousel, (i*slidesInCarrousel)+slidesInCarrousel).map((item:any, index:number)=>{  
                let favorite_content = user_favorites.find((favorite)=>{
                  if(item.id===favorite.id && title === favorite.category && page === favorite.page){
                    return favorite
                  }
                })
                if(isTop)
                {
                  ...
                  <>
                  <span style={
                    windowWidth!==undefined?{width:`${slideDimensions.width}%`, height:`${slideDimensions.height}px`}:
                    {width:`20%`, height:`139px`}
                  } 
                  ref = {modalRef} key={index} 
                  className='slidetop__netflixcarrousel'>
                    <span style={{height:`${slideDimensions.height}px`}}>
                    {imgNumbers[index+((i)*slidesInCarrousel)]}
                    </span>
                    <img style={slideWidth!==undefined?
                        {height:`${slideDimensions.height}px`}:{}} src={item.topImgLink} />
                  </span>
                  <CarouselModal 
                      changeModalState={changeModalState}
                      borderLeft={index===0?true:false}
                      borderRight={index===(movies.slice(i*slidesInCarrousel, (i*slidesInCarrousel)+slidesInCarrousel).length-1)?true:false}
                      height={typeof(modalRef.current)==="undefined"?0:modalRef.current.offsetHeight*0.8} 
                      width={typeof(modalRef.current)==="undefined"?0:modalRef.current.offsetWidth} 
                      item={item}
                      category={title}
                      page={page}
                      gatherDescription={gatherDescription}
                      addToFavorites={addToFavorites}
                      isFavorite={favorite_content===undefined?false:true}
                      />
                  </>)
                }  
                else
                {
                  ...
                  <span key={index} >
                    ...
                    <CarouselModal 
                      ...
                      borderLeft={index===0?true:false}
                      borderRight={index===(movies.slice(i*slidesInCarrousel, (i*slidesInCarrousel)+slidesInCarrousel).length-1)?true:false}
                      height={typeof(modalRef.current)==="undefined"?0:modalRef.current.offsetHeight} 
                      width={typeof(modalRef.current)==="undefined"?0:modalRef.current.offsetWidth} 
                      item={item}
                      category={title}
                      page={page}
    ...
    return(<>
      <div>
      <h3 className='carousel__title'>{title} 
        {isTop?<></>:
        <>
          <span>{"Explorar todos"} </span> <i className="ri-arrow-right-s-line"></i>
        </>
        } 
        </h3>
